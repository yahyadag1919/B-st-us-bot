"""
bist_h1_tournament.py — BIST SAATLİK (H1) STRATEJİ TURNUVASI
===============================================================
...
BAR_PERIOD = os.environ.get("BAR_PERIOD", "2y")   # yfinance saatlikte ~730 gun veriyor
BARS_PER_SESSION = int(os.environ.get("BARS_PER_SESSION", "8"))  # BIST 10:00-18:00
...
def fetch_m15(ticker):
    # NEDEN H1 (2026-08-07): M15 turnuvasinda 15 varyantin TAMAMI maliyet
    # sonrasi negatif cikti, ama cogu %35-46 isabet tutturuyordu - yani yon
    # tahminleri fena degildi, MALIYET yiyordu. M15'te tipik stop mesafesi
    # ~%1, gidis-donus maliyet %0.15; yani maliyet riskin ~%15'i. 1:2 R:R'de
    # basabas isabet %33.3 olmasi gerekirken gercek esik %38-40'a cikiyor.
    # Saatlik mumlarda stop mesafesi buyur, maliyetin payi duser.
    # EK KAZANC: yfinance saatlik veriyi 60 gun degil ~2 YIL geriye veriyor,
    # yani M15 turnuvasinin "tek piyasa rejimi" kisiti da ortadan kalkiyor.
    df = yf.Ticker(ticker).history(period=BAR_PERIOD, interval=BAR_INTERVAL)
...
STRATEGIES = [
    ...
    ("Günlük Trend + H1 pullback",       lambda d, dl=None: strat_daily_trend_pullback(d, dl, tol_pct=0.4)),
    ("Günlük Trend + H1 (geniş tol)",    lambda d, dl=None: strat_daily_trend_pullback(d, dl, tol_pct=0.8)),
]
...
def main():
    ...
    print(f"H1 TURNUVA BASLIYOR: {len(tickers)} hisse", flush=True)
    send_telegram_message(
        "🏁 [BIST H1 (SAATLİK) TURNUVA] Başladı.\n"
        f"{len(tickers)} hisse | {BAR_PERIOD} {BAR_INTERVAL} verisi | {len(STRATEGIES)} strateji\n"
    ...
    for n, tk in enumerate(tickers, 1):
        try:
            ...
            turnover = float((df["close"] * df["volume"]).mean()) * BARS_PER_SESSION
    ...
    lines = ["🏆 [BIST H1 TURNUVA SONUÇLARI]",
    ...
    lines.append("")
    lines.append(f"ℹ️ Veri aralığı: {BAR_PERIOD} ({BAR_INTERVAL} mumlar). M15 turnuvasının "
                 "60 günlük tek-rejim kısıtı burada YOK — bu aralık yükselen ve düşen "
                 "dönemleri birlikte kapsar, dolayısıyla LONG/SHORT asimetrisi çıkarsa "
                 "bu sefer rejim yanılsaması olma ihtimali çok daha düşüktür.")
    if illiquid:
